  ...
  <div class="card">
    ...
    <form id="caseForm">
      ...
      <div class="form-grid">
        ...
        <div class="field" id="hakKelasWrap" style="{{ $case->penjamin === 'BPJS Kesehatan' ? '' : 'display:none;' }}">
          <label class="req">Hak Kelas BPJS</label>
          <select name="hakKelas" id="hakKelasSel">
            <option value="">Pilih Kelas Perawatan…</option>
            <option value="Kelas 1 (Rajal)" {{ $case->hak_kelas === 'Kelas 1 (Rajal)' ? 'selected' : '' }}>Kelas 1 (Rajal)</option>
            <option value="Kelas 2 (Rajal)" {{ $case->hak_kelas === 'Kelas 2 (Rajal)' ? 'selected' : '' }}>Kelas 2 (Rajal)</option>
            <option value="Kelas 3 (Rajal)" {{ $case->hak_kelas === 'Kelas 3 (Rajal)' ? 'selected' : '' }}>Kelas 3 (Rajal)</option>
            <option value="Kelas 1 (Ranap)" {{ $case->hak_kelas === 'Kelas 1 (Ranap)' ? 'selected' : '' }}>Kelas 1 (Ranap)</option>
            <option value="Kelas 2 (Ranap)" {{ $case->hak_kelas === 'Kelas 2 (Ranap)' ? 'selected' : '' }}>Kelas 2 (Ranap)</option>
            <option value="Kelas 3 (Ranap)" {{ $case->hak_kelas === 'Kelas 3 (Ranap)' ? 'selected' : '' }}>Kelas 3 (Ranap)</option>
          </select>
        </div>
        <div class="field" id="rujukanBpjsWrap" style="{{ $case->penjamin === 'BPJS Kesehatan' ? '' : 'display:none;' }}">
          <label class="req">Rujukan Aktif BPJS</label>
          <input name="rujukanBpjs" id="rujukanBpjsInput" placeholder="Ketik nomor/detail rujukan aktif" value="{{ $case->rujukan_bpjs }}">
        </div>
      </div>
      ...
      <div style="background:var(--white); border:1px solid var(--slate-200); border-radius:8px; padding:16px; display:flex; flex-direction:column; gap:8px; margin-bottom: 20px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--slate-100); padding:8px 0;">
          ...
          <div style="display:flex; gap:20px;">
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpAnestesi" value="Ya" {{ $case->pre_op_anestesi === 'Ya' ? 'checked' : '' }} style="width:16px; height:16px; margin:0;"> Ya
            </label>
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpAnestesi" value="Tidak" {{ $case->pre_op_anestesi === 'Ya' ? '' : 'checked' }} style="width:16px; height:16px; margin:0;"> Tidak
            </label>
          </div>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--slate-100); padding:8px 0;">
          ...
          <div style="display:flex; gap:20px;">
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpLab" value="Ya" {{ $case->pre_op_lab === 'Ya' ? 'checked' : '' }} style="width:16px; height:16px; margin:0;"> Ya
            </label>
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpLab" value="Tidak" {{ $case->pre_op_lab === 'Ya' ? '' : 'checked' }} style="width:16px; height:16px; margin:0;"> Tidak
            </label>
          </div>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--slate-100); padding:8px 0;">
          ...
          <div style="display:flex; gap:20px;">
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpRad" value="Ya" {{ $case->pre_op_rad === 'Ya' ? 'checked' : '' }} style="width:16px; height:16px; margin:0;"> Ya
            </label>
            <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
              <input type="radio" name="preOpRad" value="Tidak" {{ $case->pre_op_rad === 'Ya' ? '' : 'checked' }} style="width:16px; height:16px; margin:0;"> Tidak
            </label>
          </div>
        </div>
        ...
        <div style="display:flex; flex-direction:column; gap:8px; padding:8px 0;">
          <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="display:flex; gap:20px;">
              <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
                <input type="radio" name="preOpKonsul" value="Ya" {{ $case->pre_op_konsul === 'Ya' ? 'checked' : '' }} style="width:16px; height:16px; margin:0;"> Ya
              </label>
              <label style="display:flex; align-items:center; gap:6px; font-weight:600; cursor:pointer; margin:0; font-size:13.5px;">
                <input type="radio" name="preOpKonsul" value="Tidak" {{ $case->pre_op_konsul === 'Ya' ? '' : 'checked' }} style="width:16px; height:16px; margin:0;"> Tidak
              </label>
            </div>
          </div>
          <div id="konsulDetailWrap" class="field" style="display:{{ $case->pre_op_konsul === 'Ya' ? 'block' : 'none' }}; margin-top:4px; width:100%;">
            <input type="text" name="preOpKonsulDetail" id="konsulDetailInput" value="{{ $case->pre_op_konsul_detail }}" placeholder="Ketik nama spesialisasi konsul...">
          </div>
        </div>
      </div>
...
@section('scripts')
  <script>
    ...
    let alatList = {!! json_encode($alatFromRaw) !!};
    
    let tambahanList = {!! json_encode($tambahanFromRaw) !!};
